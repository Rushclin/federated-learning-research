...
class FedavgServer(BaseServer):
    ...
    def _init_model(self, model):
        logger.info(
            f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] Initialisation du modèle!')
        init_weights(model, self.args.init_type, self.args.init_gain)
        logger.info(f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] ...Initialisation du modele reussit ({self.args.model_name})!')
        return model
    ...
    def _get_algorithm(self, model, **kwargs):
        ...
        if self.args.algorithm != 'fedsgd':
            optimizer.add_param_group(
                dict(params=list(self.global_model.buffers())))
        return optimizer
    ...
    def _create_clients(self, client_datasets):
        ...
        def __create_client(identifier, datasets):
            client = CLINET_CLASS(
                args=self.args, training_set=datasets[0], test_set=datasets[-1])
            client.id = identifier
            return client

        logger.info(
            f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] Creation des clients !')
        clients = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(int(self.args.K), os.cpu_count() - 1)) as workhorse:
            for identifier, datasets in TqdmToLogger(
                ...
                logger=logger,
                desc=f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] ...creation des client en cours... ',
                total=len(client_datasets)
            ):
                clients.append(workhorse.submit(
                    __create_client, identifier, datasets).result())
        logger.info(
            f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] ...creation de {self.args.K} clients!')
        return clients

    def _sample_clients(self, exclude=[]):
        logger.info(
            f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] Clients!')
        if exclude == []:
            num_sampled_clients = max(int(self.args.C * self.args.K), 1)
            sampled_client_ids = sorted(random.sample(
                [i for i in range(self.args.K)], num_sampled_clients))
        else:
            ...
            if num_unparticipated_clients == 0:
                ...
                num_sampled_clients = max(
                    int(self.args.eval_fraction * num_unparticipated_clients), 1)
        ...
        logger.info(f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] ...{num_sampled_clients} clients ont été selectionné!')
    ...
    def _log_results(self, resulting_sizes, results, eval, participated, save_raw):
        ...
        for identifier, result in results.items():
            client_log_string = f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] [{"EVALUATE" if eval else "UPDATE"}] [CLIENT] < {str(identifier).zfill(6)} > '
            if eval:
                loss = result['loss']
        ...
        total_log_string = f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] [{"EVALUATE" if eval else "UPDATE"}] [SUMMARY] ({len(resulting_sizes)} clients):'
        ...
        top10_indices = np.argpartition(losses_array, -int(0.1 * len(losses_array)))[-int(
            0.1 * len(losses_array)):] if len(losses_array) > 1 else 0
        top10 = np.atleast_1d(losses_array[top10_indices])
        top10_mean, top10_std = top10.dot(np.atleast_1d(
            num_samples[top10_indices])) / num_samples[top10_indices].sum(), top10.std()

        bot10_indices = np.argpartition(losses_array, max(1, int(0.1 * len(losses_array)) - 1))[
            :max(1, int(0.1 * len(losses_array)))] if len(losses_array) > 1 else 0
        bot10 = np.atleast_1d(losses_array[bot10_indices])
        bot10_mean, bot10_std = bot10.dot(np.atleast_1d(
            num_samples[bot10_indices])) / num_samples[bot10_indices].sum(), bot10.std()
        ...
        self.writer.add_scalars(
            f'Local {"Test" if eval else "Training"} Loss ' +
            eval * f'({"In" if participated else "Out"})',
            {'Avg.': weighted, 'Std.': std, 'Top 10% Avg.': top10_mean, 'Top 10% Std.': top10_std,
                'Bottom 10% Avg.': bot10_mean, 'Bottom 10% Std.': bot10_std},
            self.round
        )

        for name, val in metrics.items():
            val_array = np.array(val).astype(float)
            weighted = val_array.dot(num_samples) / sum(num_samples)
            std = val_array.std()

            top10_indices = np.argpartition(
                val_array, -int(0.1 * len(val_array)))[-int(0.1 * len(val_array)):] if len(val_array) > 1 else 0
            top10 = np.atleast_1d(val_array[top10_indices])
            top10_mean, top10_std = top10.dot(np.atleast_1d(
                num_samples[top10_indices])) / num_samples[top10_indices].sum(), top10.std()

            bot10_indices = np.argpartition(val_array, max(1, int(0.1 * len(val_array)) - 1))[
                :max(1, int(0.1 * len(val_array)))] if len(val_array) > 1 else 0
            ...
            self.writer.add_scalars(
                f'Local {"Test" if eval else "Training"} {name.title()}' +
                eval * f' ({"In" if participated else "Out"})',
                {'Avg.': weighted, 'Std.': std, 'Top 10% Avg.': top10_mean, 'Top 10% Std.': top10_std,
                    'Bottom 10% Avg.': bot10_mean, 'Bottom 10% Std.': bot10_std},
                self.round
    ...
    def _request(self, ids, eval, participated, retain_model, save_raw):
        ...
        logger.info(f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] Requete {"updates" if not eval else "evaluation"} de {"all" if ids is None else len(ids)} clients!')
        if eval:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(ids), os.cpu_count() - 1)) as workhorse:
                for idx in TqdmToLogger(
                    ...
                    jobs.append(workhorse.submit(
                        __evaluate_clients, self.clients[idx]))
                for job in concurrent.futures.as_completed(jobs):
                    results.append(job.result())
            _eval_sizes, _eval_results = list(map(list, zip(*results)))
            _eval_sizes, _eval_results = dict(
                ChainMap(*_eval_sizes)), dict(ChainMap(*_eval_results))
            ...
            logger.info(f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] ...evaluation complete de {"all" if ids is None else len(ids)} clients!')
            return None
        else:
            jobs, results = [], []
            with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(ids), os.cpu_count() - 1)) as workhorse:
                for idx in TqdmToLogger(
                    ...
                    jobs.append(workhorse.submit(
                        __update_clients, self.clients[idx]))
                for job in concurrent.futures.as_completed(jobs):
                    results.append(job.result())
            update_sizes, _update_results = list(map(list, zip(*results)))
            update_sizes, _update_results = dict(
                ChainMap(*update_sizes)), dict(ChainMap(*_update_results))
            ...
            logger.info(f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] ...mise à jour complete de {"all" if ids is None else len(ids)} clients!')
            return update_sizes

    def _aggregate(self, server_optimizer, ids, updated_sizes):
        assert set(updated_sizes.keys()) == set(ids)
        logger.info(
            f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] Aggregation!')

        coefficients = {identifier: float(nuemrator / sum(updated_sizes.values()))
                        for identifier, nuemrator in updated_sizes.items()}

        for identifier in ids:
            local_layers_iterator = self.clients[identifier].upload()
            server_optimizer.accumulate(
                coefficients[identifier], local_layers_iterator)
            self.clients[identifier].model = None
        logger.info(
            f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] ...agregation complete sur le nouveau modele!')
    ...
    def _central_evaluate(self):
        ...
        result = mm.results
        server_log_string = f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] [EVALUATE] [SERVER] '
    ...
    def update(self):
        ...
        updated_sizes = self._request(
            selected_ids, eval=False, participated=True, retain_model=True, save_raw=False)
        _ = self._request(selected_ids, eval=True,
                          participated=True, retain_model=True, save_raw=False)

        server_optimizer = self._get_algorithm(
            self.global_model, **self.opt_kwargs)
        server_optimizer.zero_grad(set_to_none=True)
        server_optimizer = self._aggregate(
            server_optimizer, selected_ids, updated_sizes)
    ...
    def evaluate(self, excluded_ids):
        if self.args.eval_type != 'global':
            ...
            _ = self._request(selected_ids, eval=True, participated=False,
                              retain_model=False, save_raw=self.round == self.args.R)
        if self.args.eval_type != 'local':
        ...
        if (not self.args.train_only) and (not self.args.eval_type == 'global'):
            ...
            for key in curr_res['clients_evaluated_out'].keys():
                for name in curr_res['clients_evaluated_out'][key].keys():
                    if 'avg' in name:
                        gap = curr_res['clients_evaluated_out'][key][name] - \
                            curr_res['clients_evaluated_in'][key][name]
                        gen_gap[f'gen_gap_{key}'] = {name: gap}
                        self.writer.add_scalars(
                            f'Generalization Gap ({key.title()})', gen_gap[f'gen_gap_{key}'], self.round)
                        self.writer.flush()
            else:
    ...
    def finalize(self):
        ...
        torch.save(self.global_model.state_dict(), os.path.join(
            self.args.result_path, f'{self.args.exp_name}.pt'))
        self.writer.close()
        logger.info(
            f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Tour: {str(self.round).zfill(4)}] ...fin!')
        if self.args.use_tb:
            input('[FINISH] ...Appuyez sur <ENTRER> pour quitter!')
